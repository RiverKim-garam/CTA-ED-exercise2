...
tidy_tweets$date <- as.Date(tidy_tweets$created_at) # making date

tidy_tweets <- tidy_tweets %>%
  arrange(date)

tidy_tweets$order <- 1:nrow(tidy_tweets) #numbering
...
tweets_nrc_sentiment_2 <- tweets_nrc_sentiment %>%
  count(date, index = order %/% 1000, sentiment) %>% # index making with order / 1000
  spread(sentiment, n, fill = 0) %>%
  mutate(sentiment = positive - negative)

tweets_nrc_sentiment_2 %>%
...
tweets_nrc_sentiment_news <- tweets_nrc_sentiment %>%
  group_by(newspaper) %>%
  count(date, index = order %/% 1000, sentiment) %>% 
  spread(sentiment, n, fill = 0) %>%
  mutate(sentiment = positive - negative)

# 1st way, point + smooth 
tweets_nrc_sentiment_news %>%
  ggplot(aes(date, sentiment, colour = newspaper, group = newspaper)) +
  geom_point(alpha=0.5) +
  geom_smooth(method= loess, alpha=0.25)
...
tweets_nrc_sentiment_news %>%
  ggplot(aes(date, sentiment)) +
  geom_point(alpha=0.5) +
  geom_smooth(aes(colour = tweets_nrc_sentiment_news$newspaper))
```

2. Build your own (minimal) dictionary-based filter technique and plot the result

```{r}
my_dic <- c('race', 'racism', 'racist', 'apartheid', 'xenocide', 'xenophobia')
value_dic <- c(1, 1, 1, 1, 1, 1)
race_dic <- data.frame(my_dic, value_dic)
race_dic

tidy_tweets %>%
  cross_join(race_dic) %>%
  group_by(date, index = order %/% 1000) %>% 
  summarise(racewords = sum(value)) %>% 
  ggplot(aes(date, racewords)) +
  geom_bar(stat= "identity") +
  ylab("race words")

# accounting total words
race_dic <- c('race', 'racism', 'racist', 'apartheid', 'xenocide', 'xenophobia')

totals <- tidy_tweets %>%
  mutate(obs=1) %>%
  group_by(date) %>%
  summarise(sum_race = sum(obs))

tidy_tweets %>%
  mutate(obs=1) %>%
  filter(grepl(paste0(race_dic, collapse = "|"),word, ignore.case = T)) %>% 
  group_by(date) %>%
  summarise(sum_racewords = sum(obs)) %>%
  full_join(totals, word, by="date") %>%
  mutate(sum_racewords= ifelse(is.na(sum_racewords), 0, sum_racewords), 
         pctracewords = sum_racewords/sum_race) %>%
  ggplot(aes(date, pctracewords)) +
  geom_point(alpha=0.5) +
  geom_smooth(method= loess, alpha=0.25) +
  xlab("Date") + ylab("% race words")
```
...
```{r}
tweets$date <- as.Date(tweets$created_at)
tweet_corpus_try <- corpus(tweets, text_field = "tweet")

#add in date and newspaper document-level information
docvars(tweet_corpus_try, "newspaper") <- tweets$newspaper
docvars(tweet_corpus_try, "date") <-tweets$date

#tokenization
toks_news_try <- tokens(tweet_corpus_try, remove_punct = TRUE)

# select only the "negative" and "positive" categories
data_dictionary_LSD2015_pos_neg <- data_dictionary_LSD2015[1:2]
toks_news_lsd_try <- tokens_lookup(toks_news_try, dictionary = data_dictionary_LSD2015_pos_neg)
print(toks_news_lsd_try)

# into document feature matrix
dfmat_news_lsd_try <- dfm(toks_news_lsd_try) %>% dfm_group(groups = newspaper)

# plot positive and negative valence over newspaper
matplot(dfmat_news_lsd_try, type = "l", lty = 1, col = 1:2, ylab = "Frequency", xlab = "") +
  grid() +
  legend("topleft", col = 1:2, legend = colnames(dfmat_news_lsd_try), lty = 1, bg = "white")
```
